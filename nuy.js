...
var req = args.join(' ')            
				...
				case 'gemes':
					...
					break
						case 'stickermeme':
						if ((isMedia && !faa.videoMessage || isQuotedImage)) {
						test = args.join(" ")
					        reply("Wait Bro Promses Dlu Yekan:V")
						var encmedia = isQuotedImage ? JSON.parse(JSON.stringify(faa).replace("quotedM", "m")).message.extendedTextMessage.contextInfo : faa
						...
						//nuy.sendMessage(from, buffer, MessageType.sticker, {quoted: faa})
							break
           case 'trigger2':
               if (!isRegistered) return nuy.sendMessage(from, `╭─「 *BELUM VERIFIKASI* 」\n❏ ${pushname}\n❏ Daftar Dulu Kak Dengan Cara\n❏ ${prefix}daftar pinky|16\n╰──────────`,MessageType.text, { quoted: faa2, contextInfo: { forwardingScore: 508, isForwarded: true}, sendEphemeral: true, thumbnail: fs.readFileSync(`./me.jpg`, 'base64')})
